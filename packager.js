...
export class Packager {
        ...
        try {
          ...
          if (verbose && !!post) {
            ...
            if (result instanceof Array)
              for (let bundle of result) console.log(bundle);
          ...
          if (ex instanceof Function) ex();
            ...
            console.log(
              `${chalk.yellow("Reason: ")}${chalk.grey(ex.message)}\r\n${ex.stack}`,
            );
      ...
      (
        await Promise.all(
          (await fs.readdir(dir, { withFileTypes: true }))
            ...
            .map(async (file) => [
              file.isFile() && path.join(dir, file.name),
              ...(file.isDirectory()
                ? await find(path.join(dir, file.name))
                : []),
            ]),
        )
      )
        ...
        .flat(Infinity)
        .filter((filename) => !!filename && filename.endsWith(".js"));
      ...
      mocha
        ...
        .then((runner) => {
          if (reporter === "base") {
            ...
            runner.on("end", () =>
              !!reporter.stats.failures ? reject(epilogue) : resolve(epilogue),
            );
          }
        });
    });
  ...
  static async rollup(src, dest, assets) {
    ...
    const input = {
      ...
      ...chunks.reduce(
        (chunks, chunk) =>
          Object.assign(chunks, { [chunk]: path.join(src, `${chunk}.js`) }),
        {},
      ),
    };
    const manualChunks = chunks.reduce(
      (chunks, chunk) =>
        Object.assign(chunks, { [chunk]: [path.join(src, `${chunk}.js`)] }),
      {},
    );
    ...
    for (let format of ["esm", "cjs"]) {
      ...
      output.push(
        ...results.map((file) =>
          format === "esm" ? file.fileName : `${format}/${file.fileName}`,
        ),
      );
    }
  ...
  static async declare(src, dest, assets) {
    const rollup = await import("rollup");
    const { generateDeclarations, filterGeneratedBundle } =
      await import("ostensibly-typed/plugin-rollup");
  ...
  switch (config.target) {
    case "clean":
      await Packager.action()("Cleaning Build Directory", [
        {
          pre: "Cleaning build directory: ",
          action: async () => await Packager.clean(Packager.paths.dist),
        },
      ]);
      break;
    ...
    case "test:ci":
      Packager.test(config.testFilter, {
        name: "json",
        options: { output: "./test/results-report.json" },
      }).finally(() => {
        process.exit(0);
      });
